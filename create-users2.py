...
def main():
    ...
    #If user wants to run the script in dry run mode
    tty = open("/dev/tty", "r")
    ...
    for line in sys.stdin:
        ...
        #Looks at the line and will skip if it is incorrectly formatted, and if records are not valid
        #Match looks at the comments and the fields looks at the number
        if match or len(fields) != 5:
            if dry_run == 'Y':
                print("Skipping invalid or comment line: %s" % (line.strip()))
            continue
        ...
        for group in groups:
            #The if statement is looking for "-".  If the input doesn't have it, the user will be added to a group
